...
def main():
    ...
    try:
        ...
        # СВЕРКА ПО ТЕКСТУ ЗАКОНА 21.09. Четыре нормы, которых в образцах
        # рунета нет, а две из них конструктор раньше терял сам.
        check("прокат: долг взыскивается надписью нотариуса (п. 3 ст. 630)",
              "исполнительной надписи нотариуса" in p and "п. 3 ст. 630" in p)
        check("прокат: возврат части платы — это ст. 630, а не 627",
              "п. 2 ст. 630" in p and "п. 3 ст. 627" in p)
        check("прокат: договор назван публичным (п. 3 ст. 626)",
              "является публичным" in p and "п. 3 ст. 626" in p)
        check("прокат: оговорка про потребительские цели не жёсткая",
              "не ограничивается потребительскими целями" in p)
        # Прокат — договор с потребителем. Условия, ограничивающие выбор
        # подсудности и навязывающие досудебный порядок, ничтожны
        # (ч. 2 ст. 16 ЗоЗПП, пп. 2 и 11). Переносить их из аренды нельзя.
        check("прокат: подсудность по выбору потребителя, а не по месту владельца",
              "по своему выбору" in p and "по месту нахождения Арендодателя. " not in p)
        check("прокат: обязательный досудебный порядок снят",
              "Претензионный порядок обязателен" not in p)
        check("аренда: обычная подсудность и претензии на месте",
              "по месту нахождения Арендодателя" in a and "Претензионный порядок обязателен" in a)
        # Прокат ведёт только тот, для кого сдача — постоянная
        # предпринимательская деятельность (п. 1 ст. 626).
        t.pick("akind", "person")
        time.sleep(0.25)
        check("прокат + физлицо: предупреждение поднято",
              t.js("document.getElementById('warn-vid').hidden") is False)
        t.pick("akind", "selfemp")
        time.sleep(0.25)
        check("прокат + самозанятый: предупреждения нет",
              t.js("document.getElementById('warn-vid').hidden") is True)

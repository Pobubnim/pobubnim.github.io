...
def visible(t):
    ...
    # ЧУЖАЯ ЦЕНА. На странице бывает сумма, которая НЕ наша услуга: домен у
    # регистратора, сервер, платный сторонний сервис. Прайс про такие не знает и
    # знать не должен, а молчать о них на странице нечестно. Разметка
    # <span data-price="foreign">200–400 ₽</span> говорит гарду «это не мы берём».
    # Метка ставится осознанно и видна в исходнике — это не обход проверки,
    # а объявление другого класса числа (21.09.2026).
    t = re.sub(r'<span[^>]*data-price="foreign"[^>]*>.*?</span>', " ", t, flags=re.S)

...
(ч. 1 ст. 10.1 Федерального закона № 152-ФЗ). Значит второй лист обязан
...
def main():
    ...
    try:
        ...
        t.set("f-paddr", "Москва, ул. Строителей, 12, кв. 5")
        ...
        check("второй лист ссылается на ч. 1 ст. 10.1", "10.1" in rasp and "отдельно от иных согласий" in rasp)
        ...
        # ч. 8 и ч. 9 — РАЗНЫЕ нормы: молчание не согласие / запреты субъекта
        # обязательны для оператора. Сверено по тексту закона 21.09.
        check("молчание не считается согласием (ч. 8 ст. 10.1)",
              "Молчание или бездействие" in rasp and "ч. 8 ст. 10.1" in rasp)
        check("запреты субъекта обязательны для оператора (ч. 9 ст. 10.1)",
              "обязательны для Оператора" in rasp and "ч. 9 ст. 10.1" in rasp)

        # Отдельность второго листа — смысл всего инструмента. «Приложение»
        # к основному согласию этому требованию НЕ отвечает: приложение есть
        # часть документа, а закон требует отдельный (ч. 1 ст. 10.1).
        t.tick("p-site", True)
        time.sleep(0.25)
        osn_pub = t.sheet("osn")
        check("второй лист назван отдельным документом, а не приложением",
              "не является приложением" in osn_pub and "ч. 1 ст. 10.1" in osn_pub)
        ...
        t.pick("who", "person")
        time.sleep(0.25)
        # Адрес оператора обязателен ВСЕГДА, включая частное лицо:
        # п. 3 ч. 4 ст. 9 требует «наименование или ФИО И АДРЕС оператора».
        check("у частного лица поле адреса тоже показано",
              t.js("document.getElementById('wrap-addr').hidden") is False)
        check("адрес оператора идёт в документ и у частного лица",
              "ул. Ленина" in t.sheet("osn"))
        t.pick("who", "org")
        ...
        rasp = t.sheet("rasp")
        check("лист распространения: прекращение передачи по ч. 12 ст. 10.1",
              "должна быть прекращена в любое время" in rasp and "ч. 12 ст. 10.1" in rasp)
        check("лист распространения: момент прекращения согласия по ч. 13",
              "ч. 13 ст. 10.1" in rasp)
        ...
        check("только фото: видеозаписи не упоминаются как результат",
              "фотографических изображений" in osn and "видеозаписей" not in osn
              and "видеозаписи с участием" not in osn)
        t.pick("media", "both")
        time.sleep(0.25)
        ...
        # ЧАСТЬ 4 СТАТЬИ 9 перечисляет, что ОБЯЗАНО быть в письменном
        # согласии. Адреса субъекта и представителя стоят там первыми
        # пунктами; без них согласие оспаривается проще всего.
        osn0 = t.sheet("osn")
        check("адрес представителя в документе (п. 2 ч. 4 ст. 9)",
              "ул. Строителей, 12" in osn0)
        check("ребёнок привязан к адресу (п. 1 ч. 4 ст. 9)",
              "проживающего по указанному адресу" in osn0)
        check("ФИО ребёнка не склоняется скриптом",
              "несовершеннолетнего ребёнка — Петров Пётр Петрович" in osn0)

        # п. 6 ч. 4 ст. 9: привлечённый фотограф обрабатывает данные ПО
        # ПОРУЧЕНИЮ и обязан быть назван в согласии. Скачанные образцы
        # этой строки не имеют вовсе.
        check("без галочки поручения пункта нет", "по поручению Оператора" not in osn0)
        t.js("(()=>{const e=document.getElementById('d-proc');if(!e.checked)e.click()})()")
        t.set("f-proc", "ИП Бубнов С. И., г. Апрелевка")
        time.sleep(0.25)
        osn1 = t.sheet("osn")
        check("поручение обработки попадает в документ",
              "ИП Бубнов С. И." in osn1 and "по поручению Оператора" in osn1)
        check("ответственность оставлена на операторе (ч. 5 ст. 6)",
              "ч. 5 ст. 6" in osn1 and "несёт Оператор" in osn1)
        t.js("document.getElementById('d-proc').click()")
        time.sleep(0.25)

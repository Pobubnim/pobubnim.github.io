...
**build_scenes.py** — сцены 8 услуг: склейка «Дальше · сцена NN» перед каждой следующей сценой и реплики автора в сценах цен и подготовки (тексты — только факты страницы и прайса), идемпотентен, блоки помечены data-gen="scenes",

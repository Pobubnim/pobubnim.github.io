...
## Качество кадра: бриф и вставки

- **Промпт = фотопостановка** (`reels/brief.py`). Сцена задаёт предмет (`image`) и по желанию
  план (`shot`), объектив (`lens`), свет (`light`), реквизит и фактуры (`set`), настроение
  (`mood`). Общий почерк серии дописывается сам: камера и оптика, грейд, признаки реализма
  (износ материалов, отпечатки, пыль, честные отражения), композиция под титры и список
  запретов (текст, лица, лишние пальцы, «пластик», HDR). `task` кладёт в задание полный бриф.
- **Всё точное рисует код** (`reels/inserts.py`), нейросеть этого не умеет: осциллограмма
  яркости по пикселям самого кадра (`scope`), сравнение «LOG | Rec.709» из того же кадра
  (`compare`), колёса Lift/Gamma/Gain (`wheels`), чек-лист (`checklist`), плашка-вывод
  (`chip`). Вставка лежит в нижней трети под титром и входит через 0,25 с.
...
python reels/reel.py import кадры.zip          # архив кадров от ChatGPT/Codex → reels/incoming/
```

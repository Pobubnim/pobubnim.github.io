...
(function () {
  ...
  function renderDogovor(d) {
    ...
    if (prokat) {
      h.push("<p>1.4. Оборудование передаётся для потребительских целей. Арендодатель в присутствии Арендатора проверяет исправность Оборудования и знакомит Арендатора с правилами его эксплуатации (ст. 628 ГК РФ).</p>");
    } else {
    ...
    if (prokat) {
      ...
      h.push("<p>2.4. Арендатор вправе отказаться от договора в любое время, письменно предупредив Арендодателя не менее чем за десять дней. В этом случае Арендодатель возвращает соответствующую часть полученной платы (ст. 627 ГК РФ).</p>");
    } else {
    ...
    if (radio("akind") === "selfemp") h.push("<p>3.2. НДС не облагается: Арендодатель применяет налог на профессиональный доход. После оплаты Арендатору передаётся чек из приложения «Мой налог».</p>");
    else if (radio("akind") === "person") h.push("<p>3.2. НДС не облагается.</p>");
    ...
    h.push("<p>7.3. Договор вступает в силу с момента подписания и действует до полного исполнения сторонами обязательств.</p>");
    h.push("<p>7.4. Споры разрешаются путём переговоров; при недостижении согласия — в суде по месту нахождения Арендодателя. Претензионный порядок обязателен, срок ответа на претензию — 10 рабочих дней.</p>");
    h.push("<p>7.5. Договор составлен в двух экземплярах, по одному для каждой стороны, имеющих равную силу.</p>");

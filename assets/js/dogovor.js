...
(function () {
  ...
  function read() {
    ...
    return {
      ...
      raw: radio("raw") === "yes", price: val("f-price"), prepay: +val("f-prepay"),
  ...
  function renderDogovor(s) {
    ...
    var priceM = money(s.price);
    var prepayM = s.price && s.prepay ? money(Math.round(+s.price * s.prepay / 100)) : null;
    var n = 0, h = [];
    ...
    h.push('<div class="doc-meta"><span>г. ' + bl(s.city, "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;") + "</span><span>«___» ____________ 20___ г.</span></div>");
    ...
    p("3.1. Стоимость Услуг составляет " + (priceM ? esc(priceM) : BL) + " ₽" + (s.executor === "selfemp" ? ". НДС не облагается: Исполнитель применяет налог на профессиональный доход" : s.executor === "fiz" ? ". НДС не облагается" : "") + ".");
    if (s.prepay > 0 && s.prepay < 100) {
      p("3.2. При подписании Договора Заказчик вносит предоплату в размере " + s.prepay + "% от стоимости Услуг" + (prepayM ? " (" + esc(prepayM) + " ₽)" : "") + ". Оставшаяся часть оплачивается " + (s.paywhen === "day" ? "в день съёмки, до её начала" : "в течение 3 (трёх) дней после передачи Материалов") + ".");
    } else if (s.prepay === 100) {
      p("3.2. Услуги оплачиваются в размере 100% стоимости при подписании Договора.");
    } else {
      p("3.2. Оплата производится " + (s.paywhen === "day" ? "в день съёмки, до её начала" : "в течение 3 (трёх) дней после передачи Материалов") + " в полном объёме.");
    }
    if (s.executor === "selfemp") {
      p("3.3. После каждой оплаты Исполнитель передаёт Заказчику чек, сформированный в приложении «Мой налог». В случае снятия с учёта в качестве плательщика НПД Исполнитель уведомляет Заказчика в течение 3 (трёх) рабочих дней.");
    } else if (s.executor === "fiz" && s.client !== "fiz") {
      p("3.3. Заказчик выступает налоговым агентом: исчисляет и удерживает НДФЛ из вознаграждения Исполнителя и уплачивает страховые взносы в соответствии с законодательством РФ.");
    }

    sec("Перенос и отмена");
    if (svc.isShoot) {
      p("4.1. Заказчик вправе один раз перенести дату съёмки на любую свободную у Исполнителя дату в пределах 6 (шести) месяцев, уведомив Исполнителя не менее чем за 14 (четырнадцать) дней, — без удержаний.");
      p("4.2. Каждая из Сторон вправе отказаться от Договора (ст. 782 ГК РФ). При отказе Заказчика Исполнитель удерживает из предоплаты сумму фактически понесённых расходов и потерь от брони даты, которую Стороны согласовали в следующем размере: при отказе более чем за 30 дней до съёмки предоплата возвращается полностью; за 14–30 дней — удерживается 30% предоплаты; менее чем за 14 дней — 50%; в день съёмки — 100%. Остаток возвращается в течение 5 (пяти) рабочих дней.");
      p("4.3. При отказе Исполнителя по причинам, не связанным с нарушениями Заказчика, предоплата возвращается полностью в течение 5 (пяти) рабочих дней.");
    } else {
      p("4.1. Каждая из Сторон вправе отказаться от Договора (ст. 782 ГК РФ). При отказе Заказчика после начала работ Заказчик оплачивает фактически выполненную часть работ пропорционально готовности, согласованной Сторонами.");
    ...
    h.push('<div class="req"><div><b>Исполнитель</b>' + execIntro(s) + "<br>Тел.: " + bl(s.execPhone) + '<div class="sig">Подпись: ' + BL + "</div></div><div><b>Заказчик</b>" + clientIntro(s) + "<br>Тел.: " + bl(s.clientPhone) + '<div class="sig">Подпись: ' + BL + "</div></div></div>");
  ...
  function renderAkt(s) {
    ...
    h.push("<p>3. Стоимость услуг составляет " + (priceM ? esc(priceM) : BL) + " ₽" + (s.executor === "selfemp" ? ", НДС не облагается (НПД)" : "") + ". Оплата произведена " + (s.paywhen === "day" || s.prepay === 100 ? "полностью" : "в размере " + BL + " ₽; остаток подлежит оплате в порядке, установленном Договором") + ".</p>");
    h.push("<p>4. Настоящий Акт составлен в двух экземплярах, по одному для каждой из Сторон.</p>");
    h.push('<div class="req"><div><b>Исполнитель</b>' + bl(s.execName) + '<div class="sig">Подпись: ' + BL + "</div></div><div><b>Заказчик</b>" + bl(s.clientName) + '<div class="sig">Подпись: ' + BL + "</div></div></div>");
    h.push(MARK_BR);
    return h.join("");
  }
  ...
  document.getElementById("btn-doc").addEventListener("click", function () {
    var title = mode === "dogovor" ? "dogovor" : "akt";
    var html = "﻿<html><head><meta charset='utf-8'><style>body{font-family:'Times New Roman',serif;font-size:12pt;line-height:1.5}h3{text-align:center}h4{margin:14pt 0 6pt}.doc-meta{display:flex;justify-content:space-between}.req{margin-top:20pt}.blank{border-bottom:1px solid #000;padding:0 40px}.doc-note{font-size:9pt;font-style:italic;color:#555}.bmark{background:#000;color:#f5efe2;font-family:Georgia,serif;font-style:italic;font-size:11pt;padding:2pt 6pt}.bmark-row{margin:6pt 0}.bmark-row.br{text-align:right}</style></head><body>" + paper.innerHTML + "</body></html>";
    var a = document.createElement("a");
    a.href = URL.createObjectURL(new Blob([html], { type: "application/msword" }));
    a.download = title + "-pobubnim.doc";
